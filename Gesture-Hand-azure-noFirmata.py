...
# REMOVE or COMMENT OUT these two lines from your old code:
...
def move_servo(angles):
    try:
        thumb = int(min(max(angles[0],0),180))
        index = int(min(max(angles[1],0),180))
        middle = int(min(max(angles[2],0),180))
        ring = int(min(max(angles[3],0),180))
        little = int(min(max(angles[4],0),180))

        data = f"{thumb},{index},{middle},{ring},{little}\n"
...
with mp_hands.Hands(
    ...
    while cap.isOpened():
        ...
        # VOICE COMMAND ACTIVATED 
            ...
            elif any(keyword in words for keyword in VOICE_KEYWORDS['bye']):
                print("👋🏻 🙋🏻‍♀️ GOOD BYE !! ⚜️")
                move_servo([0, 0, 180, 0, 0])
